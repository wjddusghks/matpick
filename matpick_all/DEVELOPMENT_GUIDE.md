...
|--------|-----------|:--------:|------|
| 또간집 | 풍자 (UCrDMtdCSMTGVmUKvuhcahRw) | 96 | EP.0~EP.95 |
| 먹을텐데 | 성시경 (UCfpaSruWW3S4dibonKXENjA) | 204 | 공식 채널 214개 영상·국내 식당 기준 검증 완료 |
| 맛있는 녀석들 | (UCT-eNSaIVbeFnMhOLPDBGhg) | 64 | 데이터 추가 검증 필요 |
| 쯔양 | (UCnkCnLLKUhHyN0ssCENfWw) | 0 | 공개 데이터 미반영 |
| **합계** | **4개 크리에이터** | **364 visits** | **342개 고유 식당** |
...
### 4.4 먹을텐데 데이터 상세 (2026-07-15 검증)

- **공식 영상**: 성시경 공식 채널에서 제목에 `먹을텐데`가 포함된 214개 영상
- **회차 기준**: 공식 영상 업로드일 오름차순(공식 제목에 일련번호가 없어서 맛픽에서 부여)
- **식당 회차**: 국내 식당이 확인된 194개 회차, 204 visits, 195개 고유 식당
- **해외 제외**: 해외 식당 15개 회차, 28곳은 맛픽·CSV에서 제외
- **지도 대조**: 국내 186곳을 카카오맵 현행 장소와 매칭
- **메뉴/가격**: 182개 장소, 689개 메뉴 스냅샷
- **영업시간**: 184개 장소의 요일별 영업시간 스냅샷
- **보류 원칙**: 폐점·이전·상호 변경 가능성 때문에 현행 장소가 확정되지 않은 식당은 영상 설명의 당시 주소를 유지
- **원본/산출물**: `source-data/mogeultende/`의 JSON 및 CSV
...
먹을텐데와 또간집 반영 이후, 남은 크리에이터 데이터의 검증이 필요합니다.

**작업 내용:**
1. **맛있는 녀석들**: 영상 제목과 식당 정보 추가 검증
2. **쯔양**: 공식 영상 목록부터 재수집해 공개 데이터 구성
...
2. **맛있는녀석들/쯔양 데이터**: 영상 제목과 식당 정보를 공식 출처 기준으로 재검증해야 함.
3. **먹을텐데 미매칭/해외 식당**: 현행 지도 장소가 확정되지 않은 국내 9곳과 해외 식당은 좌표·영업시간이 일부 비어 있음.
