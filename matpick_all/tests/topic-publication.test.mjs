...
const sixBatch = require('../client/src/data/generated/six-topic-research.generated.json');
const sixPublication = require('../../source-data/six-topic-publication-2026-09-25/publication.json');
...
test("original census keeps its existing-only feed; verified expansion adds eligible sourced restaurants", () => {
  ...
  for (const source of batch.sources) {
    ...
    const expected = new Set([...batch.sourceLinks, ...requestedBatch.sourceLinks, ...choizaBatch.sourceLinks, ...sixBatch.sourceLinks].filter(link => link.sourceId === source.id)
    ...
    for (const r of linked) {
      assert.ok(existing.has(r.id) || [...requestedBatch.restaurants, ...choizaBatch.restaurants, ...sixBatch.restaurants].some(next => data.getRestaurantById(next.id)?.id === r.id));
      ...
      const sixRestaurant = sixBatch.restaurants.find(next => data.getRestaurantById(next.id)?.id === r.id);
      const current = {...existing.get(r.id), ...requestedPatch, ...choizaRestaurant, ...sixRestaurant, ...overrides[r.id]};
